...
Block.prototype = {
    init: function () {
        ...
        _t.container.addEventListener("click", function (e) {
            ...
            e = e || window.event;      //获取事件对象，浏览器兼容设置
            var target = e.target || e.srcElement;     //获取触发事件的元素
            if (target.className.indexOf('block') != -1) {     //字符串检索
                _t.sum++;       //分数加1
                //显示分数
    ...
    //生成游戏结束界面
        ...
        if( !document.getElementById("result") ){
            var resullt = document.createElement('div');   //创建div元素
            resullt.className = 'result';     //设置类名
            resullt.id = 'result';     //设置id
            resullt.innerHTML = '<h1>GAME OVER</h1><h2> id="score">分数：'+_t.sum+'</h2><span id="restart">重新开始</span>';
            //设置HTML内容
            _t.container.parentNode.appendChild(result);   //添加元素
        }else{
            var result = document.getElementById("result");   //获取元素
            resullt.style.display = "block";    //显示元素
